...
export const AttachmentManager: React.FC<AttachmentManagerProps> = ({
    ...
    function validate(): boolean {
        ...
        if (!titleVal) {
            hasError = true;
            setTitleErrorMsg(
                `${intl.formatMessage({ id: "ATTACHMENT.NAME" })} ${intl.formatMessage({ id: "FORM.LABEL.REQUIRED" })}`
            );
        } else {
            setTitleErrorMsg("");
        }
        ...
        if (!fileVal) {
            hasError = true;
            setAttachmentErrorMsg(
                `${intl.formatMessage({ id: "LEGAL.POPUP.ATTACHMENTS" })} ${intl.formatMessage({ id: "FORM.LABEL.REQUIRED" })}`
            );
        } else {
            setAttachmentErrorMsg("");
        }
    ...
    const isAttachmentTypeError = (value: { message?: string } | boolean) => {
        ...
        if (typeof value === "object" && value?.message) {
            ...
            setAttachmentErrorTypeMsg(intl.formatMessage({ id: "LEGAL.POPUP.ATTACHMENTS.TYPE.INVALID" }));
        }
    };
    ...
    const isAttachmentSizeError = (value: boolean) => {
        if (value) {
            ...
            const msg = `${intl.formatMessage({ id: "LEGAL.POPUP.ATTACHMENTS.SIZE.HIGHER" })} ${perFileMaxAllowedSizeInMb} MB`;
            ...
            <div className="row mb-4 p-2 align-items-center">
                ...
                <div className="col-12 pt-4">
                    <div className="row align-items-center mx-0 px-0 ">
                        <div className="col mx-0 px-2 form-field-label-wrapper">
                ...
                <div className="col-12 pt-4">
                    <div className="row align-items-center mx-0 px-0 ">
                        <div className="col mx-0 px-2 form-field-label-wrapper">
                            <InfoLabels text={"LEGAL.POPUP.ATTACHMENTS"} isRequired isI18nKey />
                        </div>
                        <div className="col mx-0 px-0 ">
                            <UserAttachmentUpload
                                title="LEGAL.POPUP.ATTACHMENTS"
                        ...
                        <div className="col-1 mx-0 pb-8 px-0 d-flex justify-content-end h-100 ">
                            <button
                                type="button"
                                className="btn MOD_btn btn-create w-10 me-0 mt-5"
                                onClick={handleAdd}
                                disabled={isFileUploading}
                            >
                                <span className="detailLabels_btn-lbl-txt-medium-2-ar__H+GFf">
                                    {intl.formatMessage({ id: "LEGAL.UPLOAD" })}
                                </span>
                ...
                {/* Grid */}
                <div className="col-12 pt-4">
                    {AttachmentList && AttachmentList.length === 0 && isLoading ? <NoRecordsAvailable /> : null}

                    {AttachmentList && AttachmentList.length > 0 ? (
                        <div className="row py-5 attachment-table-header">
                            <div className="col">
                                <span>{intl.formatMessage({ id: "LEGAL.FORM.ATTACHMENT.TAB.ATTACHMENT.TITLE" })}</span>
                            </div>
                            <div
                                className="col"
                                style={{
                            ...
                            >
                                <div className="text-start">
                                    <span>{intl.formatMessage({ id: "LEGAL.FORM.ATTACHMENT.TAB.FILENAME" })}</span>
                                </div>
                            </div>
                            <div className="col">
                                <div className="text-end">
                                    <span>{intl.formatMessage({ id: "LEGAL.FORM.ATTACHMENT.TAB.ACTION" })}</span>
                                </div>
                            </div>
                        </div>
                    ) : null}
                                ...
                                <div className="d-flex gap-5 flex-end">
                                    <div>
                                        {!isDeleteLoading && loadingIndex == null ? (
                                            <Tooltip
                                                placement="bottom"
                                                title={<ManageTooltipTitle title={"LEGAL.ATTACHMENTS.TOOLTIP.DELETE"} />}
                                                arrow
