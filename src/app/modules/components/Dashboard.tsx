...
// Global interval management to persist across component mounts/unmounts
...
const Dashboard: React.FC = () => {
  ...
  const handlePeriodicUpdate = useCallback(() => {
    ...
    console.log('Periodic update called at:', new Date().toLocaleTimeString());
  }, []);

  useEffect(() => {
    // Only start the interval if it's not already active
    if (!isIntervalActive) {
      // Call the function immediately on first load
      handlePeriodicUpdate();

      // Set up interval to call function every 2 minutes (120000ms)
      globalInterval = setInterval(handlePeriodicUpdate, 120000);
      isIntervalActive = true;
    }

    // Cleanup function - but don't clear the global interval
    // The interval should continue running even when component unmounts
    return () => {
      // No cleanup here - let the interval persist
    };
  }, [handlePeriodicUpdate]);

  // Optional: Add a cleanup when the entire app unmounts (you can add this to your main App component)
  useEffect(() => {
